...
aret_logistic_ridge\$results\[103,\]

```{r}
caret_logistic_ridge$results[103,]
```
...
predicted_test_lasso <- predict(caret_logistic_lasso, test_data, type='prob')

dim(predicted_test_lasso)

head(predicted_test_lasso)
```

```{r}
# Compute the AUC

cut.obj <- cutpointr(x     = predicted_test_lasso$Smoking,
                     class = test_data$smoking)
...
```{r}
caret_logistic_lasso$results[44,]
```
...
# The first 3 predictors with the largest absolute coefficient (excluding the intercept)

head(as.matrix(coefs[ind[-1],]),4)
```

```{r}
options(scipen=99)
coefs <- coef(caret_logistic_lasso$finalModel, caret_logistic_lasso$bestTune$lambda)
coefs
